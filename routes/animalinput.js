...
router.post('/', [auth, officerauth], (req, res) => {
	...
	pool.query(animalinputquery, (err, result) => {
		...
		var animalattributes = `insert into attributes (Height,Weight,Length,Age,Sex,Color,Limbcount) values (${req.body.height},${req.body.weight},${req.body.lengthh},${req.body.age},'${req.body.sex}','${req.body.color}',${req.body.limbcount})`;
		pool.query(animalattributes, async (err, result) => {
			if (err) {
				return res.status(400).send({ error: err });
			}
			let attribid = result.insertId;
			var animalattributes = `insert into attribanimal (AnimalID,AttributeID) values (${req.body.animalid},${attribid})`;
			pool.query(animalattributes, async (err, result) => {
				if (err) {
					return res.status(400).send({ error: err });
				}
				...
				const token =
					req.header('x-auth-token') || req.cookies.AuthToken;
				const user = jwt.verify(token, process.env.jwtPrivateKey);
				let username = await User.findOne({ _id: user });

				var reportquery = `insert into reports (reportername,reporterid,reportdate)  values ('${username['name']}','${username['_id']}','${dmy}');`;
				pool.query(reportquery, (err, result) => {
					if (err) {
						return res.status(400).send({ error: err });
					}

					let repid = result.insertId;
					// console.log(repid);
					var reportanimalquery = `insert into reportanimal (AnimalID,reportid)  values (${req.body.animalid},${repid});`;
					pool.query(reportanimalquery, (err, result) => {
						if (err) {
							return res.status(400).send({ error: err });
						}
						res.send();
